...
class RankCard:
    ...
    async def card1(self, resize: int = 100) -> Union[None, bytes]:
        ...
        myFont = ImageFont.truetype(path + "/assets/levelfont.ttf", 40)
        ...
        myFont = ImageFont.truetype(path + "/assets/levelfont.ttf", 30)
        draw.text(
            (197, (327 / 2) + 125),
            f"LEVEL - {RankCard._convert_number(self.level)}",
            font=myFont,
            fill=self.text_color,
            stroke_width=1,
            stroke_fill=(0, 0, 0),
        ...
        mask_im = (
            Image.open(path + "/assets/mask_circle.jpg").convert("L").resize((170, 170))
        )
        ...
        new.paste(
            self.background,
            (0, 0),
            Image.open(path + "/assets/curvedoverlay.png").convert("L"),
        )
        ...
        if resize != 100:
            self.background = self.background.resize(
                (
                    int(self.background.size[0] * (resize / 100)),
                    int(self.background.size[1] * (resize / 100)),
                )
            )
    ...
    async def card2(self, resize: int = 100) -> Union[None, bytes]:
        ...
        myFont = ImageFont.truetype(path + "/assets/levelfont.ttf", 50)
